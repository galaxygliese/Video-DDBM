...
from dit.checkpoint import auto_grad_checkpoint
...
# from dit.ckpt_utils import load_checkpoint
...
class DiT(nn.Module):
    ...
    def __init__(
        ...
        self.x_embedder = PatchEmbed3D(patch_size, self.in_channels, embed_dim=hidden_size)
    ...
    def forward(self, x, timesteps, y=None, xT=None):
        ...
        if y is None:
            B = x.shape[0]
            y = torch.zeros((B,)).to(torch.long).to(x.device)
...
def DiT_S_2(from_pretrained=None, **kwargs):
    model = DiT(
        depth=12,
        hidden_size=384,
        patch_size=(1, 2, 2),
